...
exports.login = functions.https.onRequest((request, response) => {
  ...
  if (request.method === 'GET') {
    ...
    <html>
      ...
      <body>
        <form action="/fir-dff30/us-central1/login" method="post">
  ...
  `);
    ...
    let find = Data.find(o => o.Id == request.body.Id && o.Passowrd == request.body.Password)
    if (find) {
      // Here, you should validate the user account.
      // In this sample, we do not do that.
      const responseurl = decodeURIComponent(request.body.responseurl);
      functions.logger.log(`Redirect to ${responseurl}`);
      let er = responseurl.replace("xxxxxx", JSON.stringify(find));
      return response.redirect(er);
    } else {
      response.send(`
    <html>
      <meta name="viewport" content="width=device-width, initial-scale=1">
...
exports.fakeauth = functions.https.onRequest((request, response) => {
  functions.logger.log("sdasdasdasds")
  ...
  return response.redirect(
    `/fir-dff30/us-central1/login?responseurl=${encodeURIComponent(responseurl)}`);
});
...
exports.faketoken = functions.https.onRequest((request, response) => {
  ...
  const code = request.query.code ?
    request.query.grant_type : request.body.code;
...
app.onSync((body, headers) => {
  ...
  //console.log(JSON.parse(`"${headers.authorization.split(' ')}"`));
  let smartId = JSON.parse(headers.authorization.split(' ')[1]);
  // console.log(firebaseRef.child(smartId.smartHomeId));
  //console.log(firebaseRef.child(smartId.smartHomeId).once('value'), "ppppppppppppppppp");
  for (devicecounter = 0; devicecounter < deviceitems.length; devicecounter++) {
    if (deviceitems[devicecounter].traits.includes('action.devices.traits.TemperatureSetting')) {
      if (deviceitems[devicecounter].attributes.queryOnlyTemperatureSetting == true) {
        firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('TemperatureSetting').set({ thermostatMode: "off", thermostatTemperatureAmbient: 20, thermostatHumidityAmbient: 90 });
      } else if (deviceitems[devicecounter].attributes.queryOnlyTemperatureSetting == false) {
        firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('TemperatureSetting').set({ thermostatMode: "off", thermostatTemperatureSetpoint: 25.5, thermostatTemperatureAmbient: 20, thermostatHumidityAmbient: 90, thermostatTemperatureSetpointLow: 15, thermostatTemperatureSetpointHigh: 30 });
      }
    }
    if (deviceitems[devicecounter].traits.includes('action.devices.traits.OnOff')) {
      firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('OnOff').set({ on: false });
    }
    if (deviceitems[devicecounter].traits.includes('action.devices.traits.Brightness')) {
      firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('Brightness').set({ brightness: 10 });
    }
    if (deviceitems[devicecounter].traits.includes('action.devices.traits.ColorSetting')) {
      firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('ColorSetting').set({ color: { name: "deep sky blue", spectrumRGB: 49151 } });
    }
    if (deviceitems[devicecounter].traits.includes('action.devices.traits.Modes')) {
      ...
      firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('Modes').set({ currentModeSettings: { modename: modevalue } });
    }
    if (deviceitems[devicecounter].traits.includes('action.devices.traits.FanSpeed')) {
      firebaseRef.child(smartId.smartHomeId).child(deviceitems[devicecounter].id).child('FanSpeed').set({ currentFanSpeedSetting: 20.0 });
    }
  }
  return {
    requestId: body.requestId,
    payload: {
      agentUserId: smartId.Id,
      devices: deviceitems
    },
  };
});
...
const queryFirebase = async (deviceId, user) => {
  console.log("query Firebase ssssssssss");
  const snapshot = await firebaseRef.child(user.smartHomeId).child(deviceId).once('value');
...
const queryDevice = async (deviceId, user) => {
  console.log("query Device");
  const data = await queryFirebase(deviceId, user);
...
app.onQuery(async (body, headers) => {
  console.log("onquery");
  console.log(headers);
  let user = JSON.parse(headers.authorization.split(' ')[1]);
...
const updateDevice = async (execution, deviceId, user) => {
  console.log(deviceId, "oooooooooooooooo");
  const { params, command } = execution;
  let state; let ref;
  switch (command) {
    case 'action.devices.commands.OnOff':
      state = { on: params.on };
      ref = firebaseRef.child(user.smartHomeId).child(deviceId).child('OnOff');
      break;
    case 'action.devices.commands.BrightnessAbsolute':
      state = { brightness: params.brightness };
      ref = firebaseRef.child(user.smartHomeId).child(deviceId).child('Brightness');
      break;
    case 'action.devices.commands.ColorAbsolute':
      state = { color: params.color };
      ref = firebaseRef.child(user.smartHomeId).child(deviceId).child('ColorSetting');
      break;
    case 'action.devices.commands.SetFanSpeed':
      state = { currentFanSpeedSetting: params.fanSpeed };
      ref = firebaseRef.child(user.smartHomeId).child(deviceId).child('FanSpeed');
      break;
    case 'action.devices.commands.SetModes':
      state = { currentModeSettings: params.updateModeSettings };
      ref = firebaseRef.child(user.smartHomeId).child(deviceId).child('Modes');
      break;
    case 'action.devices.commands.ThermostatTemperatureSetpoint':
      state = { thermostatTemperatureSetpoint: params.thermostatTemperatureSetpoint };
      ref = firebaseRef.child(user.smartHomeId).child(deviceId).child('TemperatureSetting');
      break;
    ...
    for (const device of command.devices) {
      for (const execution of command.execution) {
        executePromises.push(
          updateDevice(execution, device.id, user)
            .then((data) => {
 ...
 * has been changed.
 */
exports.reportstate = functions.database.ref('{smartHomeId}/{deviceId}').onWrite(async (change, context) => {
  ...
  console.log(postData.payload.devices.states);
...
//https://c8d5-2401-4900-51db-b8f-ed51-3648-9da0-1e0c.ngrok.io/fir-dff30/us-central1/login